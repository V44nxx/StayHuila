...
new_user_menu = """        <nav class="nav-links">
            <a href="/hospedajes">Hospedajes</a>
            <a href="#">Experiencias</a>
            <a href="#">Planificador <span class="nav-badge">IA</span></a>
            <a href="#">Comunidad</a>
            <a href="/panel-anfitrion" class="host-nav-link" style="font-weight: 700; color: var(--primary);">Pon tu espacio en StayHuila</a>
        </nav>
        <div class="user-menu">
...
for html_file in glob.glob(r'c:\xampp\htdocs\StayHuila\templates\*.html'):
    ...
    if '<nav class="nav-links">' in content:
        content = re.sub(r'<nav class="nav-links">.*?</header>\s*(<script>.*?var dropdown = document.*?<\/script>)?', new_user_menu, content, flags=re.DOTALL)
        with open(html_file, 'w', encoding='utf-8') as f:
            f.write(content)
            
print("Navegación actualizada con el botón de anfitrión en nav-links")
